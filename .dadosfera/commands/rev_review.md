...
- **Routing dimension** (mutually exclusive per task for this review):
  - **Recommended to current active plan**: Tasks that advance the current objective and should live in the closest `docs/plans/active/` plan (often via `/plan_from_active_tasks_conversation`). **Detect the absolute path of this plan.**
  - **Recommended to backlog plan**: Tasks that are useful but out-of-scope for the current active plan, better tracked under `docs/plans/backlog/`. **Detect the absolute path of this plan.**
...
- Structure the response with these sections (in this order):
  - **Pending Tasks – recommended to current `/active` plan**
    - **Plan Path**: `<absolute_path_to_active_plan>` or `TBD Plan`
  - **Proposed Tasks – recommended to current `/active` plan**
    - **Plan Path**: `<absolute_path_to_active_plan>` or `TBD Plan`
  - **Pending Tasks – recommended to a `/backlog` plan**
    - **Plan Path**: `<absolute_path_to_backlog_plan>` or `TBD Plan`
...
```markdown
**Pending Tasks – recommended to current `/active` plan**
**Plan Path**: `/Users/me/projects/my-repo/docs/plans/active/QW_2h_HIGH_update_reviews.md`

1. Fix linter errors in `commands/rev_review.md` – introduced during command update
2. Update README.md to reference new review command structure – mentioned but not completed

**Proposed Tasks – recommended to current `/active` plan**
**Plan Path**: `/Users/me/projects/my-repo/docs/plans/active/QW_2h_HIGH_update_reviews.md`

3. Add pre-commit hook to validate command format – suggested as quality improvement

**Pending Tasks – recommended to `/backlog` plan**
**Plan Path**: `TBD Plan`

4. Create visual diagram of command relationships – started research but out of scope

**Proposed Tasks – recommended to `/backlog` plan**
**Plan Path**: `/Users/me/projects/my-repo/docs/plans/backlog/LI_1d_LOW_future_automation.md`

5. Automate command sync across all repos – nice-to-have for future
6. Add command versioning system – discussed as potential enhancement
```
