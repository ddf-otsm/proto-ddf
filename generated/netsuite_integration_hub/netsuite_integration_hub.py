"""
NetSuite Integration Hub - Multi-Source Data Integration Showcase
==================================================================

This application demonstrates comprehensive data integration capabilities
between multiple sources and NetSuite CRM. It serves as both a functional
integration tool and an example of what can be generated by Proto-DDF.
...
- Integration logs with timestamps and error tracking
- Responsive UI built with Reflex framework

Architecture:
- Frontend: Reflex (React-based) with modern UI components
- Backend: Python with FastAPI (via Reflex)
- State: Reactive state management with real-time updates
- Data: Simulated integration scenarios with realistic delays

This is a generated application created by Proto-DDF to showcase
AI-powered application generation capabilities.
"""
...
# Import Reflex configuration (ports, logging, etc.)
...
class State(rx.State):
    """
    Application state for NetSuite Integration Hub.

    Manages the entire application state including data sources, integration
    progress, field mappings, and synchronization statistics. Uses Reflex's
    reactive state management for real-time UI updates.
    """

    # === Integration Control ===
    selected_source: str = ""  # Currently selected data source type
    integration_status: str = IntegrationStatus.IDLE  # Current integration state
    integration_message: str = ""  # User-friendly status message
    progress: int = 0  # Sync progress percentage (0-100)

    # === Data Management ===
    source_records: List[Dict] = []  # Raw records from selected data source
    mapped_records: List[Dict] = []  # Records mapped to NetSuite format
    sync_logs: List[Dict] = []  # Integration activity log entries

    # === Statistics Tracking ===
    total_synced: int = 0  # Total records processed across all syncs
    successful_syncs: int = 0  # Number of successfully synced records
    failed_syncs: int = 0  # Number of failed sync attempts
    last_sync_time: str = ""  # Timestamp of most recent sync operation

    # === Field Mapping Configuration ===
    source_fields: List[str] = []  # Field names from current data source
    netsuite_fields: List[str] = ["Customer Name", "Email", "Phone", "Address", "Account ID"]  # Target NetSuite fields
    field_mapping: Dict[str, str] = {}  # Mapping from source to NetSuite fields

    # === Sample Data for Demonstration ===
    # Each data source type has sample records with different field naming conventions
    # to demonstrate the intelligent field mapping capabilities
    sample_data: Dict[str, List[Dict]] = {
        # CSV format: Standard column headers with underscore/id prefixes
        SourceType.CSV: [
        ...
        # JSON API format: Nested/camelCase field names typical of REST APIs
        SourceType.JSON: [
        ...
        # Database format: SQL-style column names with underscores
        SourceType.DATABASE: [
        ...
        # REST API format: API-specific field naming patterns
        SourceType.REST_API: [
        ...
        # Salesforce format: SF-specific field naming (ID prefixes, billing fields)
        ...
        # Webhook format: Event-driven data with webhook-specific fields
        SourceType.WEBHOOK: [
