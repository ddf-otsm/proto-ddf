...
class State(rx.State):
    ...
    netsuite_fields: List[str] = ["Customer Name", "Email", "Phone", "Address", "Account ID"]
    field_mapping: Dict[str, str] = {}
    
    # Sample data for different sources
    sample_data: Dict[str, List[Dict]] = {
        SourceType.CSV: [
            {"id": "1", "name": "Acme Corp", "email": "contact@acme.com", "phone": "+1-555-0101", "location": "New York"},
            {"id": "2", "name": "TechStart Inc", "email": "info@techstart.com", "phone": "+1-555-0102", "location": "San Francisco"},
            {"id": "3", "name": "Global Solutions", "email": "hello@global.com", "phone": "+1-555-0103", "location": "London"},
        ],
        SourceType.JSON: [
            {"customer_id": "JSON001", "company": "DataFlow Systems", "contact_email": "admin@dataflow.io", "tel": "+1-555-0201", "city": "Boston"},
            {"customer_id": "JSON002", "company": "Cloud Innovations", "contact_email": "support@cloudinno.com", "tel": "+1-555-0202", "city": "Seattle"},
        ],
        SourceType.DATABASE: [
            {"cust_id": "DB100", "cust_name": "Enterprise Analytics", "email_address": "sales@enterprise.com", "phone_number": "+1-555-0301", "region": "Chicago"},
            {"cust_id": "DB101", "cust_name": "Smart Retail Co", "email_address": "info@smartretail.com", "phone_number": "+1-555-0302", "region": "Austin"},
            {"cust_id": "DB102", "cust_name": "FinTech Plus", "email_address": "contact@fintech.com", "phone_number": "+1-555-0303", "region": "Miami"},
        ],
        SourceType.REST_API: [
            {"api_id": "REST50", "org_name": "Medical Systems Inc", "primary_email": "info@medsys.com", "contact_phone": "+1-555-0401", "headquarters": "Houston"},
            {"api_id": "REST51", "org_name": "EduTech Platform", "primary_email": "hello@edutech.com", "contact_phone": "+1-555-0402", "headquarters": "Portland"},
        ],
        SourceType.SALESFORCE: [
            {"sf_id": "SF7001", "account_name": "Manufacturing Pro", "email": "contact@mfgpro.com", "phone": "+1-555-0501", "billing_city": "Detroit"},
            {"sf_id": "SF7002", "account_name": "Logistics Network", "email": "info@logistics.net", "phone": "+1-555-0502", "billing_city": "Atlanta"},
            {"sf_id": "SF7003", "account_name": "Green Energy Co", "email": "sales@greenenergy.com", "phone": "+1-555-0503", "billing_city": "Denver"},
        ],
        SourceType.WEBHOOK: [
            {"webhook_id": "WH9001", "entity_name": "Digital Marketing Agency", "contact_email": "team@digitalmarket.com", "phone_num": "+1-555-0601", "address": "Los Angeles"},
            {"webhook_id": "WH9002", "entity_name": "Consulting Group", "contact_email": "info@consulting.com", "phone_num": "+1-555-0602", "address": "Washington DC"},
        ]
    }
    ...
    def connect_source(self):
        ...
        try:
            ...
            self.integration_message = f"Successfully connected! Found {len(self.source_records)} records"
    ...
    def auto_map_fields(self):
        ...
        try:
            ...
            if not self.source_fields:
                ...
                self.integration_message = "No source fields available. Please connect to a source first."
                return
    ...
    def sync_to_netsuite(self):
        ...
        try:
            logger.info(f"sync_to_netsuite called - records: {len(self.source_records)}, mapping: {self.field_mapping}")
            
            if not self.source_records:
                logger.warning("No source records available")
                self.integration_message = "No source data available. Please connect to a source first."
                return
                ...
                try:
                    ...
                    for source_field, netsuite_field in self.field_mapping.items():
                        if source_field in record:
                            ...
                            logger.debug(f"  Mapped {source_field}={record[source_field]} -> {netsuite_field}")
        ...
        rx.vstack(
            ...
            rx.card(
                rx.vstack(
                    ...
                    rx.cond(
                        ...
                        rx.text(
                            f"Last sync: {State.last_sync_time}",
                            size="2",
                            color="gray"
                        )
                    ),
            ...
            rx.cond(
                ...
                rx.card(
                    rx.vstack(
                        rx.hstack(
                            rx.heading(f"🔄 Active Integration: {State.selected_source}", size="6"),
                            rx.button(
                                "Reset",
                        ...
                        # Progress bar
                        rx.cond(
                            (State.integration_status == IntegrationStatus.CONNECTING) | (State.integration_status == IntegrationStatus.SYNCING),
                rx.vstack(
                        ...
                        rx.hstack(
                            rx.button(
                                ...
                                disabled=(State.integration_status == IntegrationStatus.CONNECTING) | (State.integration_status == IntegrationStatus.SYNCING),
                                size="3"
                            ),
                            rx.button(
                                "2. Auto-Map Fields",
            ...
            rx.cond(
                ...
                rx.card(
                    rx.vstack(
                        rx.heading(
                            rx.text("📋 Source Data (", State.source_records.length(), " records)", as_="span"),
                            size="5"
                        ),
                        rx.box(
                            rx.table.root(
            ...
            rx.cond(
                ...
                rx.card(
                    rx.vstack(
                        rx.heading(
                            rx.text("✅ Synced to NetSuite (", State.mapped_records.length(), " records)", as_="span"),
                            size="5"
                        ),
                        rx.vstack(
                            rx.foreach(
                                ...
                                rx.hstack(
                                            rx.cond(
                                                ...
                                                rx.vstack(
                                                    rx.text("Customer Name", size="1", color="gray", weight="medium"),
                                                    rx.text(record.get("Customer Name", ""), size="2", weight="bold"),
                                                    spacing="1",
                                                    align="start"
                                                )
                                            ),
                                            rx.cond(
                                                record.get("Email"),
                                                rx.vstack(
                                                    rx.text("Email", size="1", color="gray", weight="medium"),
                                                    rx.text(record.get("Email", ""), size="2", weight="bold"),
                                                    spacing="1",
                                                    align="start"
                                                )
                                            ),
                                            rx.cond(
                                                record.get("Phone"),
                                                rx.vstack(
                                                    rx.text("Phone", size="1", color="gray", weight="medium"),
                                                    rx.text(record.get("Phone", ""), size="2", weight="bold"),
                                                    spacing="1",
                                                    align="start"
                                                )
                                            ),
                                            rx.cond(
                                                record.get("Address"),
                                                rx.vstack(
                                                    rx.text("Address", size="1", color="gray", weight="medium"),
                                                    rx.text(record.get("Address", ""), size="2", weight="bold"),
                                                    spacing="1",
                                                    align="start"
                                                )
                                            ),
                                            rx.cond(
                                                ...
                                                rx.vstack(
                                                    rx.text("Account ID", size="1", color="gray", weight="medium"),
                                                    rx.text(record.get("Account ID", ""), size="2", weight="bold"),
                                                    spacing="1",
                                                    align="start"
                                                )
                                            ),
                                    ...
                                    align="center",
                                        ...
                                        rx.cond(
                                            ...
                                            rx.text(record.get("_error", ""), size="1", color="red")
                                        ),
                                        spacing="2",
                                        width="100%"
                                    ),
